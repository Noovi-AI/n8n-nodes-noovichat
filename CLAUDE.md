...
A version published to npm cannot be unpublished. **Clean tree (G1)** and
**HEAD pushed (G2)** remain mandatory before `npm publish`. The
**Mon–Fri 08:00–19:00 BRT / Friday-night window (G3)** is a Chatwoot-only
gate (shared multi-tenant image). This package is versioned opt-in — the
client updates when they choose — and is **exempt from G3**. Prefer not
to publish during business hours as common sense, but do not describe
that as a blocking golden rule. See `../docs/rules/deploy-safety.md`.
...
The root `.claude/settings.json` has a blocking hook that intercepts `npm publish` on a dirty working tree or unpushed HEAD. n8n is exempt from the Chatwoot G3 time window.
...
nodes/NooviChat/
  ...
  descriptions/              ← per-resource config (operations, parameters)
    ...
    UazapiDescription.ts        ← WhatsApp (UAZAPI)
    CaptainDescription.ts       ← Captain AI hook
    CompanyDescription.ts       ← B2B companies
    WhatsappTemplateDescription.ts ← WhatsApp templates
    WebhookDescription.ts

credentials/
  NooviChatApi.credentials.ts           ← baseUrl + apiAccessToken (Profile → API Access)
...
Operator credentials match MCP `.mcp.json.example`: Base URL + API token (Profile → API Access) + Account ID per node. Campaign/SLA are n8n-only; conversation CRUD is n8n+REST; Captain/UAZAPI/Company are on both MCP and this node.
